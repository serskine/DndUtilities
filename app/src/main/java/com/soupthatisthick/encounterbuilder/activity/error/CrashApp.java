...
public final class CrashApp {

    private final static String TAG = "CrashApp";
    ...
    private static CrashActivityConfig config = new CrashActivityConfig();
    ...
    /**
     * Installs CrashApp on the application using the default error activity.
     *
        ...
        try {
            if (context == null) {
                ...
                if (oldHandler != null && oldHandler.getClass().getName().startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
                    Log.e(TAG, "CrashApp was already installed, doing nothing!");
                } else {
                    if (oldHandler != null && !oldHandler.getClass().getName().startsWith(DEFAULT_HANDLER_PACKAGE_NAME)) {
                        Log.e(TAG, "IMPORTANT WARNING! You already have an UncaughtExceptionHandler, are you sure this is correct? If you use a custom UncaughtExceptionHandler, you must initialize it AFTER CrashApp! Installing anyway, but your original handler will not be called.");
                    }
                    ...
                    Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                        ...
                        public void uncaughtException(Thread thread, final Throwable throwable) {
                            if (config.isEnabled()) {
                                Log.e(TAG, "App has crashed, executing CrashApp's UncaughtExceptionHandler", throwable);
                                ...
                                if (hasCrashedInTheLastSeconds(application)) {
                                    ...
                                    } else if (config.getBackgroundMode() == CrashActivityConfig.BACKGROUND_MODE_SHOW_CUSTOM || !isInBackground) {
                                    ...
                                    } else if (config.getBackgroundMode() == CrashActivityConfig.BACKGROUND_MODE_CRASH) {
                                        if (oldHandler != null) {
                                            oldHandler.uncaughtException(thread, throwable);
                                            return;
                                        }
                ...
                Log.i(TAG, "CrashApp has been installed.");
            }
        } catch (Throwable t) {
            Log.e(TAG, "An unknown error occurred while installing CrashApp, it may not have been properly initialized. Please report this as a bug if needed.", t);
        }
    }
    ...
    public static String getStackTraceFromIntent(@NonNull Intent intent) {
        return intent.getStringExtra(CrashApp.EXTRA_STACK_TRACE);
    }
     ...
     */
    @Nullable
    public static CrashActivityConfig getConfigFromIntent(@NonNull Intent intent) {
        CrashActivityConfig config = (CrashActivityConfig) intent.getSerializableExtra(CrashApp.EXTRA_CONFIG);
    ...
    public static String getActivityLogFromIntent(@NonNull Intent intent) {
        return intent.getStringExtra(CrashApp.EXTRA_ACTIVITY_LOG);
    }
     ...
     */
    public static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull CrashActivityConfig config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull CrashActivityConfig config) {
     ...
     * @param config   The config object as obtained by calling getConfigFromIntent.
     */
    public static void closeApplication(@NonNull Activity activity, @NonNull CrashActivityConfig config) {
        if (config.getEventListener() != null) {
     ...
     * If you want to check the config, use CrashActivityConfig.Builder.get();
     *
     * @return the current configuration
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static CrashActivityConfig getConfig() {
        return config;
    }

    /**
     * INTERNAL method that sets the configuration of the library.
     * You must not use this, use CrashActivityConfig.Builder.apply()
     *
     * @param config the configuration to use
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull CrashActivityConfig config) {
        CrashApp.config = config;
    }
